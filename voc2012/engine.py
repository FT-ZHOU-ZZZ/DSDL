...
class Engine(object):
    ...
    def learning(self, model, criterion, train_dataset, val_dataset, test_dataset, optimizer=None):
        ...
        test_dataset.transform = self.state['val_transform']
        test_dataset.target_transform = self._state('val_target_transform')
        ...
        if self.state['evaluate']:
            print('=========> Evaluate the performance on validation set')
            # self.validate(val_loader, model, criterion)
            print('=========> Predict and save the results of testing set')
            self.evaluate(test_loader, model, criterion)
            return
    ...
    def train(self, data_loader, model, criterion, optimizer, epoch):
        ...
        for i, (input, target) in enumerate(data_loader):
            ...
            data_loader.set_postfix(loss = self.state['loss'].data)
    ...
    def evaluate(self, data_loader, model, criterion):
        # switch to evaluate mode
        with torch.no_grad():
            model.eval()
            self.on_start_epoch(False, model, criterion, data_loader)
            ...
            for i, (input, target) in enumerate(data_loader):
                ...
                # compute output
                self.state['output'], semantic, res_semantic, feature, deep_semantic = model(feature_var, inp_var)
                self.state['loss'] = criterion(self.state['output'], target_var, semantic, res_semantic, feature,
                                               deep_semantic)

                temp = self.state['output'].cuda().data.cpu().numpy()
                ...
                for row in range(temp.shape[0]):
                    for col in range(temp.shape[1]):
                        name = self.state['out'][row]
                        val = temp[row, col]
                        with open('evaluation/comp1_cls_test_' + object_categories[col] + '.txt',
                                  'a') as file:
...
class DSDLMultiLabelMAPEngine(MultiLabelMAPEngine):
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
        feature_var = torch.autograd.Variable(self.state['feature']).float()
        target_var = torch.autograd.Variable(self.state['target']).float()
        inp_var = torch.autograd.Variable(self.state['input']).float().detach()  # one hot
        ...
        # compute output
        self.state['output'], semantic, res_semantic, feature, deep_semantic = model(feature_var, inp_var)
        self.state['loss'] = criterion(self.state['output'], target_var, semantic, res_semantic, feature, deep_semantic)
